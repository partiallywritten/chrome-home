...
requestIdleCallback(async () => {
    try {
        const res = await fetch("https://raw.githubusercontent.com/partiallywritten/Nozy-NT/refs/heads/main/manifest.json");
...
function readImageFile(file, errorElement) {
    return new Promise(resolve => {
...
function readVideoFile(file, errorElement) {
    if (!file) return null;
...
function isAnimatedWebpFile(file) {
    return new Promise(resolve => {
        const reader = new FileReader();
        reader.onload = e => {
...
async function applyLocalBackgroundFile(file) {
    if (!file) return;
    if (file.type === "video/mp4" || file.type === "video/webm") {
        forceBgCapToDefault();
        const videoFile = readVideoFile(file, bgImageError);
        if (!videoFile) return;
        bgImageInput.value = "";
        bgImageInput.removeAttribute("aria-invalid");
        markUserTheme();
        await saveBgVideo(videoFile);
        const blobUrl = await getBgImage();
        setBodyBgVideo(blobUrl);
        syncBgCapSelectState();
        return;
    }
    const dataUrl = await readImageFile(file, bgImageError);
    if (!dataUrl) return;
    bgImageInput.value = "";
    bgImageInput.removeAttribute("aria-invalid");
    markUserTheme();
    ...
    if (file.type === "image/webp") {
        const animated = await isAnimatedWebpFile(file);
        ...
        if (!dims) {
            ...
            const compressed = await compressImage(dataUrl, dims.width, dims.height, 0.8);
...
async function migrateBgImageForNewCap() {
    ...
    const current = await getBgImage();
    if (!current) return;
    ...
    if (!dims) {
        ...
        await saveBgImage(current); // routes to IDB since cap is now "default"
    } else {
        ...
        if (current.startsWith("data:image/gif") || current.startsWith("data:image/webp")) {
            await saveBgImage(current);
            return;
        }
        const compressed = await compressImage(current, dims.width, dims.height, 0.8);
        setBodyBgImage(compressed);
        await saveBgImage(compressed); // routes to IDB since cap != "default"
    }
}

async function applyLocalFaviconFile(file) {
    const dataUrl = await readImageFile(file, faviconUrlError);
    if (!dataUrl) return;
...
async function exportUserTheme() {
    const theme = {
        name: "User Theme",
        ...
        bgImageEnabled: loadBool(STORAGE_KEYS.BG_IMAGE_ENABLED, true),
    ...
    if (bgImage.startsWith("data:image/")) {
        ...
        buildAndDownload(new Uint8Array(buf), bgFilenameFromMime(mime));
        return;
    }
    if (bgImage.startsWith("blob:")) {
        ...
        try {
            const r = await fetch(bgImage);
            const fetchedMime = (r.headers.get("content-type") || blobFallbackMime).split(";")[0].trim();
            const buf = await r.arrayBuffer();
            buildAndDownload(new Uint8Array(buf), bgFilenameFromMime(fetchedMime));
        } catch {
            buildAndDownload(null);
        }
        return;
...
async function importThemeFromZip(file) {
    if (!file) return;
    ...
    if (!bytes) { alert("Import failed: could not read the file."); return; }
    ...
    const themeId = /^nnt-/.test(zipName) ? zipName : "user";
    ...
    await applyThemePreset(themeForApply, themeId);

    if (bgEntry) {
        // Re-enable the background now that we are supplying it directly from the zip
        storeBool(STORAGE_KEYS.BG_IMAGE_ENABLED, true);
...
bgColorHexInput.addEventListener("input", function() {
    syncPickerFromHex(this, bgColorInput, hex => {
...
surfaceColorHexInput.addEventListener("input", function() {
    syncPickerFromHex(this, surfaceColorInput, hex => {
...
highlightColorHexInput.addEventListener("input", function() {
    syncPickerFromHex(this, highlightColorInput, hex => {
...
textColorHexInput.addEventListener("input", function() {
    syncPickerFromHex(this, textColorInput, hex => {
...
clockHiddenToggle.addEventListener("change", function() {
    storeBool(STORAGE_KEYS.CLOCK_HIDDEN, this.checked);
    applyClockVisibility();
});
dateHiddenToggle.addEventListener("change", function() {
    storeBool(STORAGE_KEYS.DATE_HIDDEN, this.checked);
    applyClockVisibility();
});
...
bgImageToggle.addEventListener("change", function() {
    storeBool(STORAGE_KEYS.BG_IMAGE_ENABLED, this.checked);
    markUserTheme();
    applyBackground();
});
...
favoritesEnabledToggle.addEventListener("change", function() {
    storeBool(STORAGE_KEYS.FAVORITES_ENABLED, this.checked);
    applyFavoritesEnabled();
});

favoritesShowAddToggle.addEventListener("change", function() {
    storeBool(STORAGE_KEYS.FAVORITES_SHOW_ADD_BTN, this.checked);
...
bgFileSizeCapInput.addEventListener("change", function() {
    const val = clampFileSizeCap(Number(this.value));
...
for (const input of [favNameInput, favUrlInput]) {
